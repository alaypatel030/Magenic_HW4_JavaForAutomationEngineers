...
public class SongDatabase {
	 ...
	 */
	public SongDatabase() {
		// A TreeSet is a type of set, a unique element array, and more info on tree
		// sets can be found here:
		// https://docs.oracle.com/javase/7/docs/api/java/util/TreeSet.html
		songArchive = new TreeSet<Song>();
		// A Hashmap is a type of map, key to value collection, and more info on hash
	...
	/**
	 * TODO FOR HOMEWORK: Complete this method Adds the given song to the song
	 * archive
	 * 
	...
	public void deleteSong(Song songToBeDeleted) {
		// First we must check if the song is in our archive, otherwise we would be
		// wasting time cause why delete what doesn't exist?
		if (songArchive.contains(songToBeDeleted)) {
			...
			for (Song song : songArchive) {
				...
				if (song.equals(songToBeDeleted)) {
					...
					// 1. Remove that song from ALL playlist as the song is no longer in the
					// database
					...
					for (String playlistName : playlists.keySet()) {
						// Calls the deleteSongFromPlaylist method which will check every song in the
					...
					// NOTE: This is a really tricky line of code but it IS needed, before reading
					// the rest think why a 'break' ('return' works too) is needed here
					// Explanation: This break will stop going through each song in the song
					// archive. If you don't stop the loop here,
					// the loop will try to grab the next song in the archive, BUT the archive as
					// been change.
					// This will throw an exception, thus you should normally escape a loop, if what
					// you are cycling through is changed.
					// You can avoid 'break'ing if you are cleaver, and in fact the
					// 'deleteSongFromPlaylist' method does this
					break;
				}
			}
		}
	...
	/**
	 * TODO FOR HOMEWORK: Throw an IllegalArgumentException when a playlist of the
	 * given name already exists Creates a new playlist with the given name
	 * 
	 * @param playlistName The playlist name
	 * @throws IllegalArgumentException if a playlist already exists with the given
	 *                                  name
	 */
	public void createPlaylist(String playlistName) throws IllegalArgumentException {
		...
		if (playlists.containsKey(playlistName)) {
			...
			throw new IllegalArgumentException(
					String.format("PlaylistName {%s} already exists. Please use differnet name.", playlistName));
		}

		// Puts the playlist name into the playlists and maps it to a new empty list of
		// songs
		playlists.put(playlistName, new ArrayList<Song>());
	}

	/**
	 * TODO FOR HOMEWORK: Complete this method to get the list of songs for the
	 * given playlist Gets the list of songs for the given playlist name
	 * 
	 * @param playlistName The playlist name
	 * @return The list of songs for the playlist if that playlist exists, and null
	 *         otherwise
	 */
	public List<Song> getPlaylist(String playlistName) {
		// TODO FOR HOMEWORK: Add the conditional for returning the playlist
		if (playlists.containsKey(playlistName)) {
			List<Song> songInPlayList = playlists.get(playlistName);
			if (songInPlayList.size() > 0) {
			...
			// Returns null if no playlist found for given playlist name
			return null;
		}
	...
	/**
	 * TODO FOR HOMEWORK: Complete this method to add a song to the playlist NOTE:
	 * You MUST handle the case where a playlist with the given name doesn't exist
	 * and when the song isn't in the archive This can be done a couple ways, but we
	 * recommend just creating the playlist/adding to the archives if it doesn't
	 * exist Don't worry about duplicate songs you can have repeated songs in the
	 * same playlist Adds a song to the playlist
	 * 
	...
	public void deleteSongFromPlaylist(Song songToBeDeleted, String playlistName) {
		...
		if (playlists.containsKey(playlistName)) {
			...
			if (listOfPlaylistSongs.contains(songToBeDeleted)) {
				// NOTE: This is cleaver way of deleting elements from a list when you know
				// there could be duplicates
				// Explanation: This is a 'reversed' for loop, since we are starting with the
				// last element in the last and moving to the beginning.
				// If we starting at the beginning of the array, we would be moving elements up
				// in index, which means we could skip checking songs
				// If we start a 0, and delete 0, the array that was at index 2 gets moved up to
				// 1. And the element at 1 is now at 0. And will NEVER be checked cause the loop
				// will next check index 1.
				// Starting at the back will avoid this, since removing element at index 2 will
				// not change the elements at index 1 or 0, which would be the only elements
				// left to check
				for (int index = listOfPlaylistSongs.size() - 1; index >= 0; index--) { // TODO DUSTIN : EXPLAIN
					// Checks if the current song at the current index is the song that should be
					// deleted
	...
	/**
	 * TODO FOR HOMEWORK: Complete this method Returns a List of models.Song object
	 * from archive, and an empty list if no songs are found
	 * 
	 * @param songName The song name
	 * @return The List of models.Song object, and an empty list if no songs are
	 *         found
	 */
	public List<Song> findSongByName(String songName) {
		// TODO FOR HOMEWORK: Remove the line below, and return a list of songs that
		// match the given song name
		List<Song> returnSongList = new ArrayList<Song>();
		
		for(Song song : getSongArchive()) {
	...
	/**
	 * TODO FOR HOMEWORK: Complete this method Returns a List of models.Song object
	 * from archive, and an empty list if no songs from the artist are found
	 * 
	 * @param artist The artist
	 * @return The List of models.Song object, and an empty list if no songs from
	 *         the artist are found
	 */
	public List<Song> findSongsByArtist(String artist) {
		// TODO FOR HOMEWORK: Remove the line below, and return a list of songs that
		// match the given artist
		List<Song> returnSongList = new ArrayList<Song>();
